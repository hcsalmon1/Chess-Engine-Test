...
namespace CEngineCopy
{
    ...
    'a','b','c','d','e','f','g','h','N'
        ...
        static int GetMoves(ref Span<int> startingSquares, ref Span<int> targetSquares, ref Span<int> tags, ref Span<int> pieces)
        {
            int moveCount = 0;
        ...
        private static int FindCaptureBlack(int targetSquare, int captureIndex)
        {
            ...
            {
                if ((bitboard_array_global[i] & Constants.SQUARE_BBS[targetSquare]) != 0)
                {
                    captureIndex = i;
                    break;
                }
            }
            bitboard_array_global[captureIndex] &= ~Constants.SQUARE_BBS[targetSquare];
            return captureIndex;
        }

        private static void MovePiecePromote(int startingSquare, int targetSquare, int piece, int promotionPiece)
        {
            bitboard_array_global[promotionPiece] |= Constants.SQUARE_BBS[targetSquare];
            bitboard_array_global[piece] &= ~Constants.SQUARE_BBS[startingSquare];
        }

        private static void RemovePiece(int piece, int targetSquare)
        {
            bitboard_array_global[piece] &= ~Constants.SQUARE_BBS[targetSquare];
        }
        ...
        static int PerftFunctions(int depth, int ply)
        {
            ...
            int moveCount = GetMoves(ref startingSquares, ref targetSquares, ref tags, ref pieces);
